#' Read a mortality table
#'
#' Reads fixed-width mortality tables and returns a tidy data frame.
#' @param file Path to a mortality table text file.
#' @return A data frame of mortality information.
#' @export
readMortTable <- function(file) {
  ...
  mortality.table$Age <- gsub("+", "", mortality.table$Age, fixed = TRUE)
