#' Reverse geocode coordinates to political areas
#'
#' Queries the Data Science Toolkit API for political boundaries.
...
#' @param type Boundary type (e.g., "county", "country").
#' @return A character matrix of matching boundary names and codes.
#' @export
getLocationFromCoord <- function(latitudes, longitudes,
	...
	coord2pol <- function(x) {
		output <- RCurl::getURL(paste0("http://www.datasciencetoolkit.org/coordinates2politics/", 
														rjson::toJSON(x), ""))
