#' Create a Google Charts Sankey HTML file
#'
#' Writes an HTML file for a Sankey diagram using Google Visualization.
#' @param switch A data frame with origin, destination, and percent columns.
#' @param origin.date Label to append to origin nodes.
#' @param dest.date Label to append to destination nodes.
...
makeSankey <- function(switch, origin.date, dest.date, file = "sankey.html") {
	...
	party.colours <- c(Conservative = "#0087dc", Labour = "#d50000", `Lib Dem` = "#FDBB30", 
										 ...
										 DK = "#a4a6a8", Other = "#a4a6a8", BNP = "#000000")
