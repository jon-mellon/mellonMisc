...
test_that("grid_arrange_shared_legend combines plots", {
  p1 <- ggplot2::ggplot(data.frame(x = 1, y = 1, g = "a"),
                        ggplot2::aes(x, y, color = g)) +
    ggplot2::geom_point()
  p2 <- ggplot2::ggplot(data.frame(x = 1, y = 2, g = "b"),
                        ggplot2::aes(x, y, color = g)) +
    ggplot2::geom_point()
...
test_that("tif builds information plots with a custom S3 method", {
  plot_fun <- function(x, type = "IIC", ...) {
    cbind(z = c(-1, 0, 1), item1 = c(1, 2, 3), item2 = c(2, 1, 0))
  }
  assign("plot.fake_model", plot_fun, envir = globalenv())
  on.exit(rm("plot.fake_model", envir = globalenv()), add = TRUE)
  out <- tif(structure(list(), class = "fake_model"))
